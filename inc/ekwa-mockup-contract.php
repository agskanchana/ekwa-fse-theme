...
function ekwa_mockup_ai_prompts() {
	...
	$rules  = "RULES:\n"
		...
		. "- ICONS — two kinds, two rules:\n"
			. "  1. ICON FONTS: Font Awesome 6 ONLY — <i class=\"fa-solid fa-phone\" aria-hidden=\"true\"></i> for regular icons and <i class=\"fa-brands fa-facebook-f\"></i> for logos. Do NOT use Remix Icon (ri-*), Bootstrap Icons (bi-*), Themify (ti-*), Ionicons, Material Icons, Line Awesome or Glyphicons, and do NOT add a stylesheet/CDN link for any icon font — the theme bundles Font Awesome 6 Free and loads nothing else, so those icons render as blank boxes on the live site. Use only icons that exist in the FREE set (Solid and Brands); check at fontawesome.com/search with the Free filter on. Put each font icon in its own <i> element (never a background image or a ::before glyph) so it converts to an editable icon block.\n"
			. "  2. INLINE SVG ICONS: KEEP THEM EXACTLY AS THEY ARE. If an icon is written as inline <svg>…</svg> markup, do NOT swap it for a Font Awesome <i> and do not hunt for the \"nearest\" FA equivalent — the converter maps inline <svg> to the theme's ekwa/svg block and renders your markup as-is, so a custom or brand-specific icon survives 1:1. Font Awesome is the fallback for icon FONTS, not a replacement for SVG artwork you already have; forcing an SVG into FA loses the exact shape and often has no true equivalent. Keep the <svg> element's own class, width/height, viewBox and aria-hidden/focusable attributes so your CSS still targets it. (An <img src=\"icon.svg\"> is an image, not inline SVG — that converts as an image, which is also fine.)\n"
		. "- SCROLL / REVEAL ANIMATIONS — ship an EDITOR-VISIBILITY OVERRIDE with them. Animating elements in as they scroll into view is fine and encouraged, but the pattern is: a class starts hidden in CSS (opacity:0, visibility:hidden, transform:translateY(30px), clip-path, a paused animation…) and JavaScript adds a second class when it scrolls into view. That JavaScript does NOT run inside the WordPress block editor, so every one of those sections sits at its hidden start state there — the editor shows empty boxes, and the client cannot see or click the content to edit it. Keep the animation exactly as designed and add ONE override block at the very END of the stylesheet:\n"
			. "    /* Editor visibility — the reveal JS never runs in the block editor. */\n"
			. "    .wp-block .lift,  .wp-block.lift,\n"
			. "    .wp-block .fade-up, .wp-block.fade-up {\n"
			. "      opacity: 1 !important;\n"
			...
			. "    }\n"
			. "  Three things make it correct: (1) list EVERY class you used as an animation start state — .lift and .fade-up above are just examples, use your own names; (2) give each class BOTH forms, `.wp-block .x` for when the class is on an element inside a block and `.wp-block.x` for when it is on the block's own wrapper — one without the other misses half the cases; (3) put the block LAST in the file. `.wp-block` is a class the block editor puts on every block wrapper and that exists NOWHERE on the live site, so this override is invisible to visitors and the animation still plays for them exactly as designed.\n"
	...
	return array(
		'retrofit' => array(
			...
			'prompt' => "You are adapting an existing HTML mockup so it is compatible with the Ekwa WordPress theme's Mockup Converter. The theme has dynamic blocks that render real site data (menu, phone, address, hours, social, copyright, logo, search, map) from WordPress settings.\n\n"
				...
				. "THREE EDITS YOU ARE ALLOWED (AND EXPECTED) TO MAKE, because they change no pixels on the live site but decide whether the mockup converts and stays editable:\n"
				...
				. "3. APPEND THE EDITOR-VISIBILITY CSS described in the rules below. If my stylesheet hides anything that JavaScript later reveals on scroll, add that one override block at the end of the CSS — do not remove or rewrite my animations, just add the block. It only affects the WordPress editor.\n"
