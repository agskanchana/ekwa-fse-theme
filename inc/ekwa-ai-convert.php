...
- The PRIMARY site navigation menu (the main header nav, with its dropdowns/megamenu) → <!-- wp:ekwa/header-menu /-->, and COPY THE MOCKUP'S CLASS NAMES INTO ITS "classMap" so the mockup's menu CSS still matches. The block re-renders the menu from WordPress at runtime with its own canonical classes; classMap adds the mockup's alongside them, position by position. Read each value off the corresponding element of the source nav: nav (the <nav>), menu (top <ul>), item (top <li>), hasChildren (extra classes on an <li> that has a dropdown), link (top <a>), label (the <span> around the link text), caret (the dropdown arrow element), submenu (nested <ul>), submenuItem, submenuLink, megaParent (extra classes on the mega <li>), mega (the mega panel), megaGrid, megaColumn, megaImageWrap, megaImage, megaHeading, megaList, megaItem, megaLink. Omit any key the source has no extra class for, and NEVER include state classes (active, current-*) or classes the block emits itself (ekwa-*, menu-item*, sub-menu, has-image). Also set "caretTag":"i" when the arrow is an <i> icon, and "wrapLabel":false when link text sits directly in the <a> with no <span>. Example: <!-- wp:ekwa/header-menu {"classMap":{"nav":"main-nav","menu":"nav-list","item":"nav-item","link":"nav-link","hasChildren":"has-dropdown","submenu":"dropdown","megaParent":"mega-menu-parent","mega":"mega-dropdown","megaGrid":"mega-dropdown-inner","megaColumn":"mega-column","megaHeading":"dropdown-header"},"caretTag":"i"} /-->
  Secondary link lists (footer "Quick Links", "Services" columns) are NOT the main menu — convert those as ordinary lists (see the list rule below).
...
- Headings <h1>–<h6> → core/heading with the matching level. Paragraphs → core/paragraph. Copy the inner text/HTML exactly.
- Lists → ekwa/div, NOT core/list: <ul>/<ol> become ekwa/div {"tagName":"ul"|"ol"} and EACH <li> becomes a nested ekwa/div {"tagName":"li"}, with the item's contents as its own child blocks. core/list would freeze the whole list as one rich-text blob, so an icon or a link inside an item never becomes a real block and per-item classes stop being editable. Keep every class on each level.
  <!-- wp:ekwa/div {"tagName":"ul","className":"footer-links"} -->
  <!-- wp:ekwa/div {"tagName":"li","className":"footer-links__item"} -->
  <!-- wp:ekwa/link {"text":"About us","url":"/about/"} /-->
  <!-- /wp:ekwa/div -->
  <!-- /wp:ekwa/div -->
- <img> → ekwa/image with src, alt, width, height.
- An inline <svg> that is not the site logo (icons, dividers, decorative artwork) → ekwa/svg {"svg":"<svg …>…</svg>"} with the ENTIRE element copied verbatim — its class, width/height, viewBox and aria-hidden included, so the mockup's CSS still targets it. NEVER rewrite an inline SVG icon as a Font Awesome <i>: the icon rule below is about icon FONTS, and an SVG already converts 1:1.
...
- KEEP EVERY CLASS ON THE CORE BLOCKS (heading, paragraph, quote, table). A static block renders its SAVED HTML, so the class must be written on the element itself, not only in the attributes: <!-- wp:heading {"className":"section-title"} --><h2 class="wp-block-heading section-title">…</h2><!-- /wp:heading -->. Setting className without repeating it on the element silently loses the mockup's styling. (The ekwa/* blocks are server-rendered, so for those the className attribute alone is enough.)
