...
function ekwa_admin_enqueue( $hook ) {
	...
	// AI Site Generator (AI tab).
	wp_enqueue_script(
		'ekwa-ai-site-js',
		get_template_directory_uri() . '/assets/js/ekwa-ai-site.js',
		array(),
		wp_get_theme()->get( 'Version' ),
		true
	);
	$site_css = function_exists( 'ekwa_tokens_mockup_css' ) ? (string) ekwa_tokens_mockup_css() : '';
	if ( strlen( $site_css ) > 80000 ) {
		$site_css = substr( $site_css, 0, 80000 );
	}
	wp_localize_script( 'ekwa-ai-site-js', 'ekwaAiSite', array(
		'planUrl'            => esc_url_raw( rest_url( 'ekwa/v1/ai-site-plan' ) ),
		'partUrl'            => esc_url_raw( rest_url( 'ekwa/v1/ai-site-part' ) ),
		'applyUrl'           => esc_url_raw( rest_url( 'ekwa/v1/ai-site-apply' ) ),
		'manifestRebuildUrl' => esc_url_raw( rest_url( 'ekwa/v1/media-manifest-rebuild' ) ),
		'nonce'              => wp_create_nonce( 'wp_rest' ),
		'hasKey'             => (bool) ( function_exists( 'ekwa_get_ai_api_key' ) && ekwa_get_ai_api_key() ),
		'siteCss'            => $site_css,
		'i18n'               => array(
			'working'        => __( 'Working…', 'ekwa' ),
			'manifestCounts' => __( '%1$s images, %2$s videos tracked.', 'ekwa' ),
			'error'          => __( 'Request failed.', 'ekwa' ),
			'retry'          => __( 'Retry', 'ekwa' ),
			'planning'       => __( 'Planning the site…', 'ekwa' ),
			'generatingStep' => __( 'Generating %1$s of %2$s: %3$s…', 'ekwa' ),
			'done'           => __( 'All parts generated. Review each preview below, then apply.', 'ekwa' ),
			'doneWithErrors' => __( 'Finished, but %s part(s) failed — use Retry next to them.', 'ekwa' ),
			'applied'        => __( 'Applied.', 'ekwa' ),
			'view'           => __( 'View', 'ekwa' ),
			'edit'           => __( 'Edit', 'ekwa' ),
			'noKey'          => __( 'Add a Gemini API key above first.', 'ekwa' ),
			'noDescription'  => __( 'Describe the site first.', 'ekwa' ),
			'noParts'        => __( 'Tick at least one thing to generate.', 'ekwa' ),
			'copyMarkup'     => __( 'Copy block markup', 'ekwa' ),
			'copied'         => __( 'Copied!', 'ekwa' ),
			'maxImages'      => __( 'Maximum of 6 screenshots.', 'ekwa' ),
			/* translators: %s: file name. */
			'notImage'       => __( '"%s" is not an image.', 'ekwa' ),
			/* translators: %s: file name. */
			'imageTooLarge'  => __( '"%s" is larger than 4 MB.', 'ekwa' ),
			'removeImage'    => __( 'Remove', 'ekwa' ),
			'groupHeader'    => __( 'Header', 'ekwa' ),
			'groupFooter'    => __( 'Footer', 'ekwa' ),
			'groupHome'      => __( 'Home page', 'ekwa' ),
			/* translators: %s: inner page title. */
			'groupInner'     => __( 'Inner page — %s', 'ekwa' ),
			'applyLabels'    => array(
				'header' => __( 'Apply as site header', 'ekwa' ),
				'footer' => __( 'Apply as site footer', 'ekwa' ),
				'home'   => __( 'Create home page', 'ekwa' ),
				'inner'  => __( 'Create inner page', 'ekwa' ),
			),
		),
	) );
}
	...
	<div class="wrap ekwa-settings-wrap">
		...
		<form method="post" action="" class="ekwa-settings-form" id="ekwa-main-settings-form">
			...
			<div class="ekwa-tab-pane <?php echo 'ai' === $active_tab ? 'active' : ''; ?>" data-tab="ai">
				...
				<div class="ekwa-section" id="ekwa-ai-site-generator">
					<h2><?php esc_html_e( 'AI Site Generator', 'ekwa' ); ?></h2>
					<p class="description" style="margin-bottom:1em;">
						<?php esc_html_e( 'Generate a whole starter site — header, footer, home page, and one inner page — built from ekwa blocks. The AI combines your description below with the Business settings, Design Setup tokens, Project memory, and the media manifest (your real uploads), then generates each part one at a time for you to preview and apply.', 'ekwa' ); ?>
					</p>
					<table class="form-table">
						<tr>
							<th><label for="ekwa-ai-site-desc"><?php esc_html_e( 'Describe the site', 'ekwa' ); ?></label></th>
							<td>
								<textarea id="ekwa-ai-site-desc" rows="4" class="large-text" placeholder="<?php esc_attr_e( 'e.g. A modern, warm dental practice site. Highlight cosmetic dentistry and same-day crowns. Friendly but professional tone. Home page should push online booking; inner page about the doctor.', 'ekwa' ); ?>"></textarea>
								<p class="description"><?php esc_html_e( 'What the business does, what to emphasize, and the feel you want. The more specific, the better the plan.', 'ekwa' ); ?></p>
							</td>
						</tr>
						<tr>
							<th><?php esc_html_e( 'Reference screenshots', 'ekwa' ); ?></th>
							<td>
								<button type="button" class="button" id="ekwa-ai-site-add-images"><?php esc_html_e( 'Add screenshots', 'ekwa' ); ?></button>
								<input type="file" id="ekwa-ai-site-images-input" accept="image/png,image/jpeg,image/webp,image/gif" multiple hidden>
								<div id="ekwa-ai-site-images" style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px;"></div>
								<p class="description"><?php esc_html_e( 'Optional — mockups or sites whose look you want to match. Sent to both the planner and every section generator. Up to 6 images, 4 MB each (PNG/JPG/WebP/GIF). You can also paste a screenshot straight into the description box.', 'ekwa' ); ?></p>
							</td>
						</tr>
						<tr>
							<th><?php esc_html_e( 'What to generate', 'ekwa' ); ?></th>
							<td>
								<label style="margin-right:16px;"><input type="checkbox" class="ekwa-ai-site-part-cb" value="header" checked> <?php esc_html_e( 'Header', 'ekwa' ); ?></label>
								<label style="margin-right:16px;"><input type="checkbox" class="ekwa-ai-site-part-cb" value="footer" checked> <?php esc_html_e( 'Footer', 'ekwa' ); ?></label>
								<label style="margin-right:16px;"><input type="checkbox" class="ekwa-ai-site-part-cb" value="home" checked> <?php esc_html_e( 'Home page', 'ekwa' ); ?></label>
								<label style="margin-right:16px;"><input type="checkbox" class="ekwa-ai-site-part-cb" value="inner" checked> <?php esc_html_e( 'Inner page', 'ekwa' ); ?></label>
								<br>
								<input type="text" id="ekwa-ai-site-inner-topic" class="regular-text" style="margin-top:8px;" placeholder="<?php esc_attr_e( 'Inner page topic (optional — e.g. About Us, Dental Implants)', 'ekwa' ); ?>">
							</td>
						</tr>
						<tr>
							<th><label for="ekwa-ai-site-model"><?php esc_html_e( 'AI Model', 'ekwa' ); ?></label></th>
							<td>
								<?php $site_models = function_exists( 'ekwa_ai_generate_allowed_models' ) ? ekwa_ai_generate_allowed_models() : array( 'gemini-2.5-flash' => 'Gemini 2.5 Flash' ); ?>
								<select id="ekwa-ai-site-model">
									<?php foreach ( $site_models as $model_id => $model_label ) : ?>
										<option value="<?php echo esc_attr( $model_id ); ?>" <?php selected( 'gemini-2.5-flash', $model_id ); ?>><?php echo esc_html( $model_label ); ?></option>
									<?php endforeach; ?>
								</select>
								<p class="description"><?php esc_html_e( 'Used for the plan and every part. A full site is roughly 8–12 AI calls; Pro gives the best quality.', 'ekwa' ); ?></p>
							</td>
						</tr>
						<tr>
							<th><?php esc_html_e( 'Media manifest', 'ekwa' ); ?></th>
							<td>
								<?php $mm_counts = function_exists( 'ekwa_media_manifest_counts' ) ? ekwa_media_manifest_counts() : array( 'images' => 0, 'videos' => 0 ); ?>
								<span id="ekwa-ai-site-manifest-status">
									<?php
									printf(
										/* translators: 1: image count, 2: video count. */
										esc_html__( '%1$s images, %2$s videos tracked.', 'ekwa' ),
										(int) $mm_counts['images'],
										(int) $mm_counts['videos']
									);
									?>
								</span>
								<button type="button" class="button" id="ekwa-ai-site-manifest-rebuild"><?php esc_html_e( 'Rebuild manifest', 'ekwa' ); ?></button>
								<p class="description"><?php esc_html_e( 'The manifest updates itself on every upload, edit, and delete, and is sent to the AI so your real images and videos are placed into the design instead of placeholders. Give your uploads good alt text — the AI uses it to decide where each file belongs.', 'ekwa' ); ?></p>
							</td>
						</tr>
					</table>
					<p>
						<?php $site_has_key = ( defined( 'EKWA_GEMINI_API_KEY' ) && EKWA_GEMINI_API_KEY ) || get_option( 'ekwa_gemini_api_key' ); ?>
						<button type="button" class="button button-primary button-hero" id="ekwa-ai-site-generate"<?php echo $site_has_key ? '' : ' disabled'; ?>><?php esc_html_e( 'Generate Site', 'ekwa' ); ?></button>
						<span id="ekwa-ai-site-status" style="margin-left:10px;"></span>
						<?php if ( ! $site_has_key ) : ?>
							<br><em><?php esc_html_e( 'Add a Gemini API key above to enable this.', 'ekwa' ); ?></em>
						<?php endif; ?>
					</p>
					<div id="ekwa-ai-site-progress" hidden style="margin-top:8px;"></div>
					<div id="ekwa-ai-site-results" hidden style="margin-top:8px;"></div>
				</div>
				<div class="ekwa-section">
